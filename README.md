...
- **상세 개념학습:** 모든 요소를 한 문장 정의·쉬운 설명·핵심 공식·변수 뜻·유형별 적용 범위·실무 사용 사례로 구성하고, 요소별 복수 원문 링크를 외부 브라우저로 연결합니다.
- **본문 주석과 화면 복원:** 학습 구절에 형광펜·밑줄·코멘트를 저장하고, 시스템 뒤로가기와 백그라운드 복귀 시 이전 화면 흐름을 복원합니다.
- **대단원별 용어집:** 전체 소주제를 대단원 카테고리로 나눠 용어와 수식 변수 의미를 보여주며, 학습 완료 체크와 별 북마크·북마크 전용 보기를 제공합니다.
...
콘텐츠 DB와 사용자 DB는 역할이 다릅니다. 앱을 업데이트할 때 콘텐츠 자산은 교체될 수 있지만, 동일한 application ID와 동일한 서명 키로 더 높은 `versionCode`를 설치하면 사용자 DB는 유지되어야 합니다. 사용자 DB schema 5로 올리기 전에는 검증된 N-1 복사본을 남기며, schema 1·2·3·4 백업은 순서대로 schema 5 형식으로 변환해 가져옵니다. 구형 백업에 없던 개인 메모·본문 주석·용어 상태는 구형 백업을 불러와도 삭제하지 않습니다. 앱 삭제나 데이터 삭제 전에는 별도의 수동 백업도 반드시 만드세요. 백업은 일관된 DB snapshot으로 만들며, 앱이 다시 읽을 수 없는 파일을 만들지 않도록 export와 import 모두 25MB로 제한합니다.
