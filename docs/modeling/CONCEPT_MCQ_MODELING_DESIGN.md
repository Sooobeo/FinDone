...
새 원본 DB가 추가되면 같은 코드가 사실 추출, 후보 생성, 랭킹, 자동 검증과 변경 영향 분석까지 반복한다. 자동 검수에서 걸린 예외만 Owner가 확인하고, 검수 입력 해시에 묶인 배치 승인이 없는 실험이나 문항은 앱 DB에 릴리스할 수 없다.
...
### 5단계: 자동 검수·Owner 예외 확인과 최종 릴리스

- 랭커 합의도·4/5위 경계·약지도 타당성·변경 영향 자동 평가
- reference baseline에서 보수·완화 검수 프로필을 validation으로 비교
- 자동 차단은 수정하고 불안정 예외만 Owner 결정 저장
- 검수 입력 SHA-256에 묶인 Owner 배치 승인 후 앱 DB 재빌드

완료 조건: `release_ready` 실험, 0건의 안전성·자동 검수 차단, 0건의 미확인 예외, 유효한 Owner 배치 승인, 재현 가능한 SQLite SHA-256이 존재한다.
...
자동 검수는 별도 생성형 모델이 아니라 선택된 랭커와 전체 validation 실험 행렬을 이용하는 정책 계층이다. 문항별로 다음 신호를 계산한다.

- 실험 조합별 Top-4 Jaccard 합의도
- 선택 오답별 실험 조합 지지율
- 선택 4위와 비선택 후보 사이의 정규화 점수 경계
- 선택 오답의 약지도 또는 검토자 타당성 등급
- 이전 문항은행 대비 대상 요소·선택지·문항 fingerprint 변경

검수 임계값은 `reference-balanced`를 기준으로 보수·완화 방향의 ablation을 만들고 validation 예외율이 운영 목표에 가장 가까운 프로필을 선택한다. 프로필 선택에는 test를 사용하지 않으며 모든 프로필의 입력값, validation 검토량, 선택 근거를 실험 Markdown에 남긴다.
