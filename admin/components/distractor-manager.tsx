  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        title="오답 후보"
        description="랜덤 개념문제에 사용할 틀린 선택지와 틀린 이유만 요소별로 관리합니다. 문제 템플릿은 다루지 않습니다."
        actions={<button className="button button-primary" type="button" onClick={createNew}><Plus size={16} /> 오답 후보 추가</button>}
      />
      ...
      <section className="split-workspace distractor-workspace">
        <div className="panel list-pane">
          ...
          <div className="distractor-list">
            ...
            {!filtered.length ? <div className="pane-empty"><CircleOff size={22} /><strong>등록된 후보가 없습니다</strong><p>상단 버튼으로 첫 오답 후보를 추가하세요.</p></div> : null}
          </div>
        </div>
        ...
        <div className="panel detail-pane">
          {draft ? (
            <>
              <div className="detail-pane-heading"><div><p className="eyebrow">DISTRACTOR DETAIL</p><h2>{draft.id.startsWith("new-") ? "새 오답 후보" : draft.elementTitle}</h2></div><span className={draft.active ? "toggle-label on" : "toggle-label"}><button type="button" role="switch" aria-checked={draft.active} onClick={() => update("active", !draft.active)}><i /></button>{draft.active ? "앱에서 사용" : "사용 안 함"}</span></div>
              <div className="detail-form">
                <label className="editor-field"><span>연결할 학습 요소</span><div className="select-wrap full-select"><select value={draft.elementId} disabled={!draft.id.startsWith("new-")} onChange={(event) => changeElement(event.target.value)}>{elements.map((element) => <option key={element.elementId} value={element.elementId}>{element.elementId} · {element.title}</option>)}</select><ChevronDown size={14} /></div></label>
                <label className="editor-field"><span>오답 문구</span><textarea className="textarea-tall" value={draft.text} onChange={(event) => update("text", event.target.value)} rows={5} placeholder="정답처럼 보일 수 있지만 명확히 틀린 문장을 입력하세요." /></label>
                <label className="editor-field"><span>왜 틀렸는지</span><textarea className="textarea-tall" value={draft.rationale} onChange={(event) => update("rationale", event.target.value)} rows={5} placeholder="학습자가 혼동한 지점을 한 문장으로 설명하세요." /></label>
                <div className="two-field-row">
                  <label className="editor-field"><span>혼동 유형</span><input value={draft.confusionType} onChange={(event) => update("confusionType", event.target.value)} placeholder="예: 방향성 오류" /></label>
                  <label className="editor-field"><span>난이도</span><div className="select-wrap full-select"><select value={draft.difficulty} onChange={(event) => update("difficulty", event.target.value as Difficulty)}><option>기초</option><option>보통</option><option>심화</option></select><ChevronDown size={14} /></div></label>
                </div>
              ...
              <div className="detail-footer"><span><i className={dirty ? "dirty-indicator is-dirty" : "dirty-indicator"} />{dirty ? "저장하지 않은 변경" : "변경 없음"}</span><div className="detail-footer-actions">{!draft.id.startsWith("new-") ? <button className="button button-ghost-danger" type="button" onClick={remove} disabled={saving || readOnly}><Trash2 size={15} /> 삭제</button> : null}<button className="button button-primary" type="button" onClick={save} disabled={saving || !draft.text.trim() || !draft.rationale.trim()}>{saving ? "저장 중…" : "오답 후보 저장"}</button></div></div>
            </>
          ) : (
