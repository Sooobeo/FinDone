  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (signupError) {
      setError(signupError.message.includes("already registered")
        ...
        : "회원가입을 완료하지 못했습니다. 입력값을 확인하거나 잠시 후 다시 시도해 주세요.");
      return;
    }
    if (data.session) {
  ...
  return (
    <div className="login-card">
      ...
      <form className="login-form" onSubmit={submit}>
        ...
        <button className="button button-primary login-submit" type="submit" disabled={loading || configError || Boolean(success)}>
          {loading ? "가입 중…" : "Viewer로 가입"}
          <ArrowRight size={17} />
        </button>
      </form>
