...
`SUPABASE_SECRET_KEY`는 서버의 `/api/content/stable`, 최초 import와 별도 Worker 같은 신뢰 환경에서만 사용합니다. 브라우저 공개 변수에 넣거나 `NEXT_PUBLIC_` 접두사를 붙이면 안 됩니다.
...
URL과 파일 등록은 비공개 Storage 및 작업 큐에 연결되고, Source Worker가 원본 검증·URL snapshot·PDF/Office/표/텍스트 parser·필요 페이지 OCR·fragment/FTS·요소 후보 대조를 실제로 수행합니다. Admin 목록은 실제 작업 상태를 3초마다 갱신해 모든 지연 단계에서 스피너·단계명·진행률을 표시합니다. 개념 revision 검증 Worker와 승인본 SQLite 릴리스 Worker는 별도 권한으로 분리되어 있고, 후자는 기준 DB에 승인 revision을 투영한 뒤 135개 요소·FTS·해시·SQLite 무결성을 재검증합니다. 검증에 성공하면 `stable` 채널까지 자동 공개되며 Android는 다음 실행 때 새 DB를 검증 후 교체합니다.
